...
- **god-mode/sites** – Astro sites (jumpstartscaling, chrisamaya) with React components
...
### Multi-Tenant Sites
- jumpstartscaling.com (static), chrisamaya.work (tenant template, API-driven)
...
│   │   ├── tenant/        # Primary template for API-resolved tenants (chrisamaya.work, etc.)
│   ├── router.js             # JFactory proxy
│   └── scripts/              # Coolify config, run-schema-via-api
├── exports/                  # JSON seed data
├── docs/
├── _archive/                 # Legacy Directus, frontend, scripts
└── README.md
```
